...
#define CAN_XLNX_MODE_CHANGE_RETRIES  1000
#define CAN_XLNX_MODE_CHANGE_DELAY_US 1
...
{
	for (int i = 0; i < CAN_XLNX_MODE_CHANGE_RETRIES; i++) {
		if ((can_xlnx_read(dev, CAN_XLNX_SR) & mask) != 0U) {
			return 0;
		}
		k_busy_wait(CAN_XLNX_MODE_CHANGE_DELAY_US);
	}
...
/* Program and enable the controller with the stored timing and mode. */
static int can_xlnx_hw_start(const struct device *dev)
{
	...
	can_xlnx_write(dev, CAN_XLNX_BTR,
		       FIELD_PREP(CAN_XLNX_BTR_SJW, timing->sjw - 1U) |
			       FIELD_PREP(CAN_XLNX_BTR_TS2, timing->phase_seg2 - 1U) |
			       FIELD_PREP(CAN_XLNX_BTR_TS1,
					  timing->prop_seg + timing->phase_seg1 - 1U));
...
{
	struct can_xlnx_data *data = dev->data;
	can_state_change_callback_t callback = data->common.state_change_cb;
	...
	if (callback != NULL) {
		...
		callback(dev, state, err_cnt, data->common.state_change_cb_user_data);
	}
}
...
static int can_xlnx_restart(const struct device *dev)
{
	...
	if (data->common.started && data->recovering) {
		key = irq_lock();
